...
# --- Analytic CVA for FX Forward ---

def garman_kohlhagen_price(notional, S, K, T, sigma, r_d, r_f, option_type="call"):
    """
    Price an FX option using the Garman-Kohlhagen model (Black-Scholes for FX).
    
    Parameters:
    - notional: Notional amount (in domestic currency).
    - S: Spot exchange rate (e.g., USD/EUR).
    - K: Strike rate.
    - T: Time to maturity.
    ...
    Returns:
    - option_price: Price of the FX option.
    """
    # Forward rate: F = S * exp((r_d - r_f) * T)
    F = S * np.exp((r_d - r_f) * T)
    d1 = (np.log(F / K) + 0.5 * sigma**2 * T) / (sigma * np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)
    
    if option_type == "call":
        price = notional * (F * np.exp(-r_d * T) * norm.cdf(d1) - K * np.exp(-r_d * T) * norm.cdf(d2))
    else:  # put
        price = notional * (K * np.exp(-r_d * T) * norm.cdf(-d2) - F * np.exp(-r_d * T) * norm.cdf(-d1))
    
    return max(price, 0)

def analytic_cva_fx_forward_unilateral(notional, K, T, S, r_d, r_f, sigma, survival_times, survival_probs):
    """
    Calculate the analytic unilateral CVA for an FX forward.
    ...
    - r_f: Foreign interest rate.
    - sigma: Implied volatility.
    - survival_times, survival_probs: Counterparty survival data.
    ...
    P_T = np.interp(T, survival_times, survival_probs)
    default_prob = P_t0 - P_T
    fx_call_price = garman_kohlhagen_price(notional, S, K, T, sigma, r_d, r_f, option_type="call")
    cva = (1 - recovery) * default_prob * fx_call_price
    return cva

def analytic_cva_fx_forward_bilateral(notional, K, T, S, r_d, r_f, sigma, survival_times_c, survival_probs_c, survival_times_b, survival_probs_b):
    """
    Calculate the analytic bilateral CVA for an FX forward.
    ...
    recovery_b = 0.4
    fx_call_price = garman_kohlhagen_price(notional, S, K, T, sigma, r_d, r_f, option_type="call")
    fx_put_price = garman_kohlhagen_price(notional, S, K, T, sigma, r_d, r_f, option_type="put")
    ...
    for i in range(len(times) - 1):
        ...
        P_ti_plus_1_c = np.interp(t_i_plus_1, survival_times_c, survival_probs_c)
        P_ti_b = np.interp(t_i, survival_times_b, survival_probs_b)
        P_ti_plus_1_b = np.interp(t_i_plus_1, survival_times_b, survival_probs_b)
        default_prob_c = P_ti_plus_1_c - P_ti_c
        default_prob_b = P_ti_plus_1_b - P_ti_b
        cva_term += default_prob_c * P_ti_b
        dva_term += default_prob_b * P_ti_c
    
    cva = (1 - recovery_c) * cva_term * fx_call_price
    dva = (1 - recovery_b) * dva_term * fx_put_price
    bcva = cva + dva
    return bcva
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Label("Strike Rate for FX Forward (USD/EUR):"),
        dcc.Input(id="fx-strike", type="number", value=1.10, step=0.01),
        html.Label("FX Forward Maturity (Years):"),
        dcc.Input(id="fx-T", type="number", value=1.0, step=0.5),
        html.Label("Implied Volatility (%):"),
        dcc.Input(id="volatility", type="number", value=10, step=1),
...
@app.callback(
    ...
    [Input("entity-id", "value"), Input("notional", "value"), Input("fixed-rate", "value"),
     Input("strike-rate", "value"), Input("caplet-T", "value"), Input("fx-strike", "value"),
     Input("fx-T", "value"), Input("volatility", "value"), Input("lambda-guess", "value"),
     Input("interval-component", "n_intervals"), Input("theme-toggle", "value")]
)
def update_graphs(entity_id, notional, fixed_rate, strike_rate, caplet_T, fx_strike, fx_T, volatility, lambda_guess, n_intervals, theme):
    ...
    if entity_id in real_time_data:
        ...
        r_d = data["interest_rate"]  # Domestic interest rate (USD)
    ...
    T_caplet = caplet_T if caplet_T is not None else 1.5
    K_fx = fx_strike if fx_strike is not None else 1.10
    T_fx = fx_T if fx_T is not None else 1.0
    sigma = (volatility if volatility is not None else 10) / 100
    lambda_guess = (lambda_guess if lambda_guess is not None else 2) / 100
    alpha = 0.5  # Semi-annual period for caplet
    f = 0.025  # Forward rate for caplet (simplified)
    S = 1.12  # Spot exchange rate USD/EUR (simplified)
    r_f = 0.005  # Foreign interest rate (EUR)
    ...
    times, lambdas, steps = bootstrap_survival_probabilities(notional, spreads, maturities, r_d)
    ...
    cva_swap = analytic_cva_swap(notional, K_swap, r_d, t_bootstrapped, P_bootstrapped, swap_maturities, sigma)
    cva_caplet_unilateral = analytic_cva_caplet_unilateral(notional, K_caplet, T_caplet, f, r_d, sigma, alpha, t_bootstrapped, P_bootstrapped)
    cva_caplet_bilateral = analytic_cva_caplet_bilateral(notional, K_caplet, T_caplet, f, r_d, sigma, alpha, t_bootstrapped, P_bootstrapped, t_bank, P_bank)
    cva_fx_unilateral = analytic_cva_fx_forward_unilateral(notional, K_fx, T_fx, S, r_d, r_f, sigma, t_bootstrapped, P_bootstrapped)
    cva_fx_bilateral = analytic_cva_fx_forward_bilateral(notional, K_fx, T_fx, S, r_d, r_f, sigma, t_bootstrapped, P_bootstrapped, t_bank, P_bank)
    ...
    results = [
        ...
        html.H3("Analytic CVA for FX Forward"),
        html.P(f"CVA (Unilateral): ${cva_fx_unilateral:.2f}"),
        html.P(f"CVA (Bilateral): ${cva_fx_bilateral:.2f}"),
        html.P(f"Interpretation: The unilateral CVA is the expected loss due to counterparty default, weighted by the FX call option value. The bilateral CVA includes the DVA term, weighted by the FX put option value, reflecting the two-way default risk.")
    ]
