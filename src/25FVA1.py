...
def calculate_exposure_fva(epe, ene, short_term_spread, long_term_spread):
    ...
    spreads = np.array([spread_curve(t) for t in time_points])
    discount_factors = np.exp(-0.03 * time_points)  # Using 3% discount rate
    
    # Convert EPE/ENE to arrays matching time_points
    epe_array = np.full_like(time_points, epe)
    ene_array = np.full_like(time_points, ene)
    
    # FVA calculation
    dt = np.gradient(time_points)
    fva_cost = np.sum(epe_array * spreads * discount_factors * dt)
    fva_benefit = np.sum(ene_array * spreads * discount_factors * dt * (1 - 0.4))  # 40% recovery
